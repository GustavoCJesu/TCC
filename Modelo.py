...
assert 'RUL' not in col_sensores_uteis, 'ERRO: RUL esta na lista de normalização'
...
# split POR MOTOR (nao por janela) — impede que janelas do mesmo
# motor caiam em treino e validacao ao mesmo tempo (vazamento)
motores = df['unit_number'].unique()
m_treino, m_val = train_test_split(motores, test_size=0.2, random_state=0)
...
print('Motores treino: ', len(m_treino), '| Motores validacao: ', len(m_val))
...
modelo = Sequential([
    LSTM(64, return_sequences=True, input_shape=(Janela,21)),
    Dropout(0.2),
    LSTM(32),
    Dropout(0.2),
    Dense(1)
